...
Item {
  ...
  function activateWindow(toplevel) {
    ...
    if (address && Hyprland.usingLua)
      ...
      return
    // A floating window sits on top of the tiled stack but not above other
  ...
  // Bring a window to the top of the z-order by explicit address. hyprctl's
  // "bring_to_top" ignores its window argument and raises whatever is focused
  // at the time; each dispatch opens its own socket, so the two dispatches are
  // not ordered. Using alter_zorder (which carries the address) raises the
  // right window no matter which dispatch lands first.
  function raiseToTop(address) {
