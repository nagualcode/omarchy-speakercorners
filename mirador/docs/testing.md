...
| [`tests/tst_corner_mirador_modes.qml`](../tests/tst_corner_mirador_modes.qml) | [`../../Speakercorners.qml`](../../Speakercorners.qml) | Bottom-right corner Mirage trigger state machine: mode "1" (single current workspace) → mode "2" (full overview) → close, single-presentation open payload, non-corner entry points keeping the plain toggle, and the `cycle()`/`diagnose()` IPC verification surface. |
