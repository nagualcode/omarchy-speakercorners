...
| [`tests/tst_corner_mirador_modes.qml`](../tests/tst_corner_mirador_modes.qml) | [`../../Speakercorners.qml`](../../Speakercorners.qml) | Bottom-right corner Mirage trigger state machine: mode "1" (single current workspace) → mode "2" (full overview) → close, single-presentation open payload, and non-corner entry points keeping the plain toggle. |
